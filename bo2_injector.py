...
map_list = ["Select Map", "Nuketown", "Meltdown", "Hijacked", "Express", "Carrier",
			"Overflow", "Slums", "Aftermath", "Cargo", "Turbine",
			"Drone", "Raid", "Standoff", "Plaza", "Yemen"]

map_val = ["mp_crash", "mp_nuketown_2020", "mp_meltdown", "mp_hijacked",
			"mp_express", "mp_carrier", "mp_overflow", "mp_slums",
			"mp_la", "mp_dockside", "mp_turbine", "mp_drone",
			"mp_raid", "mp_village", "mp_nightclub", "mp_socotra"]			
...
def changeName_preset1():
	with verrou:
		n_name = "^1YouTube^7/^6DarkModZOFW"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
		print("Name changed !")

def changeName_preset2():
	with verrou:
		n_name = "^1YouTube^7/^2Nexocube"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
		print("Name changed !")

def changeName_preset3():
	with verrou:
		n_name = "^1YouTube^7/^5LetsTimeOut"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
		print("Name changed !")

def changeName_preset4():
	with verrou:
		n_name = "^1YouTube^0/^1KeyZiro"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
		print("Name changed !")

def changeName_preset5():
	with verrou:
		n_name = "^2\|/^1WeeD^2\|/"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
...
def changeName_preset8():
	with verrou:
		n_name = "^H  ui_globe^H  ui_globe"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
		print("Name changed !")

def changeName_preset9():
	with verrou:
		n_name = "^H/menu_mp_title_screen"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
		print("Name changed !")
		
def changeNamepresetcircle():
	with verrou:
		n_name = "^H/menu_mp_lobby_circle"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
		print("Name changed to", n_name)

def changenamepersettest():
	with verrou:
		n_name = "^Hui_smoke"
		n_name = n_name + "\x00"
		tcp.writestr(name_ptr, n_name)
...
root = Tk()
root.title("Nexocube Injector 1.3")
note = Notebook(root)
...
def r_fov():
	_Cbuf_AddText("cg_fov 65")
	change_text("^2FoV Reset")
...
def fl_bodyset():
	global varbody
	if varbody == 0:
		_Cbuf_AddText("phys_gravity 800")
		change_text("^2Floating Body ON")
		varbody = 1
	else:
		_Cbuf_AddText("phys_gravity -800")
		change_text("^1Floating Body OFF")
		varbody = 0

def func_plrinfrared():
	global infraredplayer
	if infraredplayer == 0:
		tcp.pokemem(0x20FE084, 0x2C030001)
		tcp.pokemem(0x21419DC, 0x2C030001)
		change_text("mod ON")
		infraredplayer = 1
	else:
		tcp.pokemem(0x20FE084, 0x2C030000)
		tcp.pokemem(0x21419DC, 0x2C030000)
		change_text("mod OFF")
		infraredplayer = 0
...
def set_minigun():
	...
	tcp.pokemem(0x1210887C, 0x48091111)

def set_peeckeeper():
	tcp.pokemem(0x1210887C, 0)
	tcp.pokemem(0x1210887C, 0x4C0003E8)
...
def set_map():
	map_index = map_list.index(map_var.get())
	map_val_t = map_val[map_index]
	print(map_val_t)
	_Cbuf_AddText("map " + map_val_t)

def dlmodweb():
	webbrowser.open("https://mega.nz/file/nx9VGIaC#Q_voCNpM8CoCq7R-4AV9Buz9WNoepqfP4Nho2PjY6NE")
"""
def primammounlimited():
	global isUnlimitedprim
	if isUnlimitedprim == 0:
		tcp.pokemem(0x112395AC, 0x00000FFF)
		change_text("Mod ON")
		isUnlimitedprim = 1
	else:
		tcp.pokemem(0x112395AC, 0x00800000)
		change_text("Mod Off")
		isUnlimitedprim = 0

def secammounlimited():
	global isUnlimitedsec
	if isUnlimitedsec == 0:
		tcp.pokemem(0x112395B4, 0x00000FFF)
		change_text("Mod On")
	else:
		change_text("Mod Off")

def mortalfragunlimited():
	global isFragUnlimited
	if isFragUnlimited == 0:
		tcp.pokemem(0x112395B0, 0x00000FFF)
		change_text("Mod On")
	else:
		change_text("Mod Off")
"""
def refilprimammozm():
	tcp.pokemem(0x112395AC, 0x000000FF)
	change_text("^2Primary Ammo Set")

def refilsecammozm():
	tcp.pokemem(0x112395B4, 0x000000FF)
	change_text("^2Secondary Ammo Set")

def refilmortalfragzm():
	tcp.pokemem(0x112395B0, 0x00000FFF)
	change_text("^2Mortal Frag Set")

def god_mod_zm():
	global isgodmodzm
	if isgodmodzm == 0:
		tcp.pokemem(0x23FB7E0, 0x4E800020)
		change_text("^2God Mod All On")
		isgodmodzm = 1
	else:
		tcp.pokemem(0x23FB7E0, 0x9421FBC8)
		change_text("^1God Mod All Off")
		isgodmodzm = 0

def setfaker():
	tcp.pokemem(0x110BD910, 0x000000FF)
	change_text("^2Fake Round Set")

def gtsp():
	_Cbuf_AddText("startSingleplayer")

def gtmp():
	_Cbuf_AddText("startMultiplayer")

def gtzm():
	_Cbuf_AddText("startZombies")

def thperson():
	global isThP
	if isThP == 0:
		tcp.pokemem(0x219E2C0, 0x2C030001)
		change_text("^2Third Person")
		isThP = 1
	else:
		tcp.pokemem(0x219E2C0, 0x2C030000)
		change_text("^1First Person")
		isThP = 0

def func_bigname():
	global isBigName
	if isBigName == 0:
		_Cbuf_AddText("cg_overheadNamesSize 2")
		change_text("^2Big Name On")
		isBigName = 1
	else:
		_Cbuf_AddText("cg_overheadNamesSize")
		change_text("^1Big Name Off")
		isBigName = 0

#zombie count = 1189F9F4
...
blkblank = Label(tab1, text=" ")
blkblank.grid(row=5, column=0)

dlmodpack = Button(tab1, text="Download Mod Pack", command=dlmodweb, compound=CENTER)
dlmodpack.grid(row=6, column=0)
...
blblbl = Label(tab2, text=" ")
blblbl.grid(row=5, column=1)

btngomp = Button(tab2, text="Go To Multiplayer", command=gtmp)
btngomp.grid(row=6, column=0)

btngosp = Button(tab2, text="Go To Singleplayer", command=gtsp)
btngosp.grid(row=6, column=1)

btngozm = Button(tab2, text="Go To Zombie", command=gtzm)
btngozm.grid(row=6, column=2)
...
map_var = StringVar()
map_list_om = OptionMenu(tab2, map_var, *map_list)
map_list_om.grid(row=12, column=0)
...
flb = IntVar()
flbody = Checkbutton(tab3, text="Floating Body", command=fl_bodyset, var=flb)
flbody.grid(row=1, column=1)

lwg = IntVar()
aff = Checkbutton(tab3, text="Infrared Player", command=func_plrinfrared, var=lwg)
aff.grid(row=1, column=2)

varthp = IntVar()
thirdperson = Checkbutton(tab3, text="Thrid Person", command=thperson, var=varthp)
thirdperson.grid(row=2, column=0)

varbn = IntVar()
bigname = Checkbutton(tab3, text="Big Names", command=func_bigname, var=varbn)
bigname.grid(row=2, column=1)

enable_hbm = Button(tab3, text="Enable Home Button", command=EnableHBM_Everywhere)
enable_hbm.grid(row=3, column=0)

endgame_b = Button(tab3, text="End Game", command=endgame)
endgame_b.grid(row=3, column=1)

reset_wiiu_b = Button(tab3, text="Restart Game (temp)", command=reset_wiiu)
reset_wiiu_b.grid(row=3, column=2)

shut_wiiu_b = Button(tab3, text="Shutdown WiiU", command=shutdown_wiiu)
shut_wiiu_b.grid(row=4, column=1)

blank19 = Label(tab3, text="")
blank19.grid(row=5, column=0)

n_fov = StringVar()

c_fov_e = Entry(tab3, textvariable=n_fov)
c_fov_e.grid(row=6, column=0)

c_fov_b = Button(tab3, text="Change FOV", command=change_fov)
c_fov_b.grid(row=6, column=1)

r_fov = Button(tab3, text="Reset FOV", command=r_fov)
r_fov.grid(row=6, column=2)

blank47 = Label(tab3, text="")
blank47.grid(row=7, column=0)
...
def ShowStats():
	...
	with verrou:
		...
		o_clantag = str(struct.pack("<I", int(str(tcp.readmem(stats + CLANTAG, 4)).encode("hex"), 16))).encode("hex")

		label_l = ["Deaths","Headshots","Kills","Loses","Prestige","Rank", "Score", "Wins", "XP", "Time played (s)", "Clan-Tag"]
		label_v = [o_deaths, o_hs, o_kills, o_loss, o_prestige, o_rank, o_score, o_wins, o_xp, o_time, o_clantag]
...
wp_list = OptionMenu(tab8, n_camo, *camo_list)
wp_list.grid(row=0, column=3)
...
set_minigun_b = Button(tab8, text="Set Peeckeeper as Prim.", command=set_peeckeeper)
set_minigun_b.grid(row=4, column=1)
...
c = IntVar()
c_name = Checkbutton(tab9, text="Rainbow", var = c)
c_name.grid(row=1, column=0)

name = StringVar()
name.set("New name")
t_name = Entry(tab9, textvariable=name, width=12)
t_name.grid(row=1, column=1)

b_name = Button(tab9, text="Change name", command=changeName)
b_name.grid(row=1, column=2)

ytblto = Button(tab9, text="YouTube/LetsTimeOut", command=changeName_preset3)
ytblto.grid(row=2, column=0)

ytbnexo = Button(tab9, text="YouTube/Nexocube", command= changeName_preset2)
ytbnexo.grid(row=2, column=1)

ytbadil = Button(tab9, text="YouTube/KeyZiro", command=changeName_preset4)
ytbadil.grid(row=2, column=2)

bln2 = Label(tab9, text=" ")
bln2.grid(row=3, column=0)

ytbme = Button(tab9, text="YouTube/DarkModZOFW", command=changeName_preset1)
ytbme.grid(row=4, column=0)

weed = Button(tab9, text="\|/WeeD\|/", width="17", command=changeName_preset5)
weed.grid(row=4, column=1)

blbln = Label(tab9, text="Freeze Name :", foreground="red")
blbln.grid(row=5, column=0)

globe = Button(tab9, text="Normal Globe", command=changeName_preset6)
globe.grid(row=6, column=0)

tnglobe = Button(tab9, text="TinyGlobe X2", command=changeName_preset8)
tnglobe.grid(row=6, column=1)

spookiename = Button(tab9, text="Spooky (ACOG-Logo)", command=changeName_preset7)
spookiename.grid(row=6, column=2)

blbln = Label(tab9, text=" ")
blbln.grid(row=7, column=0)

bo2name = Button(tab9, text="Black Ops 2", command=changeName_preset9)
bo2name.grid(row=8, column=0)

circlename = Button(tab9, text="Circle Logo", command=changeNamepresetcircle)
circlename.grid(row=8, column=1)

############### ZOMBIE TAB ###############
def max_points():
	tcp.pokemem(0x1123E798, 0x00FFFFFF)


hostlabel = Label(tab10, text="Host Options:", font=("Courier", 10), foreground="red")
hostlabel.grid(row=0, column=0)

uap = IntVar()
primary_ammo_check = Button(tab10, text="Refill Ammo Prim", command=refilprimammozm)
primary_ammo_check.grid(row=1, column=0)

#Wrong offset

uas = IntVar()
sec_ammo_check = Button(tab10, text="Refill Ammo Sec", command=refilsecammozm)
sec_ammo_check.grid(row=1, column=1)

umf = IntVar()
mortal_ammo_check = Button(tab10, text="Unlimited Mortal Frag", command=refilmortalfragzm)
mortal_ammo_check.grid(row=1, column=2)

btnfaker = Button(tab10, text="Set Fake Round 255", command=setfaker)#need to add command for that
btnfaker.grid(row=2, column=0)

vgmzm = IntVar()
gmodcheck = Checkbutton(tab10, text="God Mod", command=god_mod_zm, var=vgmzm)
gmodcheck.grid(row=3, column=0)

mxpoint = Button(tab10, text="Max Point", command=max_points)
mxpoint.grid(row=2, column=1)
...
note.add(tab9, text = "Name-Changer")
note.add(tab10, text = "Zombie")
note.pack()
root.mainloop()
